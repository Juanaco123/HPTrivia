...
class BookQuestions {
  ...
  let savePath: URL = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0].appending(path: "BookStatuses")
  ...
  func saveStatus() {
    do {
      let data: Data = try JSONEncoder().encode(books)
